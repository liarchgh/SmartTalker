...
//	private static String TAG = Test.class.getSimpleName();
	...
	public Voice2Text(Activity activity, EditText editText) {
		this.activity = activity;
		initLayout();
		// 初始化识别无UI识别对象
		// 使用SpeechRecognizer对象，可根据回调消息自定义界面；
		mIat = SpeechRecognizer.createRecognizer(this.activity, mInitListener);

		// 初始化听写Dialog，如果只使用有UI听写功能，无需创建SpeechRecognizer
		// 使用UI听写功能，请根据sdk文件目录下的notice.txt,放置布局文件和图片资源
		mIatDialog = new RecognizerDialog(this.activity, mInitListener);

		mSharedPreferences = this.activity.getSharedPreferences(IatSettings.PREFER_NAME, Activity.MODE_PRIVATE);
		mToast = Toast.makeText(this.activity, "", Toast.LENGTH_SHORT);
//		mResultText = ((EditText) this.activity.findViewById(R.id.voice_et));
	...
	private void initLayout() {
		SpeechUtility.createUtility(this.activity, SpeechConstant.APPID + "=5a6ac2f1");
		// this.activity.findViewById(R.id.voice_bt).setOnClickListener(this.activity);
		// this.activity.findViewById(R.id.voice_play).setOnClickListener(this.activity);
		...
		// 目前仅支持在线听写
//		RadioGroup group = (RadioGroup) this.activity.findViewById(R.id.radioGroup);
...
//		});
	...
	public void voice2Text() {
		...
		// 如何判断一次听写结束：OnResult isLast=true 或者 onError
			...
			// 设置参数
			setParam();
			boolean isShowDialog = mSharedPreferences.getBoolean(this.activity.getString(R.string.pref_key_iat_show),
					true);
			if (isShowDialog) {
				// 显示听写对话框
...
//	};
	...
	public void setParam() {
		...
		this.activity.getString(R.string.pref_key_translate, false);
		this.mTranslateEnable = mSharedPreferences.getBoolean("trnaslate", false);
		if (mTranslateEnable) {
...
//		// 开放统计 移动数据统计分析
//		FlowerCollector.onResume(Test.this);
...
//		FlowerCollector.onPause(Test.this);
//		super.onPause();
//	}

	public void setParam2Voice() {
		// 清空参数
		mIat.setParameter(SpeechConstant.PARAMS, null);
...
//
// public class Voice2Text {
// private static String appId = "5a6ac2f1";
// private static String TAG = Test.class.getSimpleName();
// // 语音听写对象
// private SpeechRecognizer mIat;
// // 语音听写UI
...
// Log.d(TAG, results.getResultString());
// if( mTranslateEnable ){
// Log.i("v2t", results.getResultString() );
// }else{
// Log.i("v2t", results.getResultString() );
// }
//
...
// * 听写UI监听器
// */
// private RecognizerDialogListener mRecognizerDialogListener = new
...
////
//// @Override
//// public void onContactQueryFinish(final String contactInfos, boolean
// changeFlag) {
//// // 注：实际应用中除第一次上传之外，之后应该通过changeFlag判断是否需要上传，否则会造成不必要的流量.
//// // 每当联系人发生变化，该接口都将会被回调，可通过ContactManager.destroy()销毁对象，解除回调。
//// // if(changeFlag) {
...
//// private void printTransResult (RecognizerResult results) {
//// String trans =
// JsonParser.parseTransResult(results.getResultString(),"dst");
//// String oris = JsonParser.parseTransResult(results.getResultString(),"src");
////
//// if( TextUtils.isEmpty(trans)||TextUtils.isEmpty(oris) ){
//// showTip( "解析结果失败，请确认是否已开通翻译功能。" );
...
//
//
// public Voice2Text(Activity context) {
// this.activity = context;
// init();
// }
//
// private void init() {
// SpeechUtility.createUtility(this.activity, SpeechConstant.APPID
// +"=5a6ac2f1");
// mEngineType = SpeechConstant.TYPE_CLOUD;
//
// // 初始化识别无UI识别对象
...
// mToast = Toast.makeText(this.activity, "", Toast.LENGTH_SHORT);
// mResultText = ((EditText) activity.findViewById(R.id.voice_et));
// }
// /**
// * 参数设置
// *
...
// mIat.setParameter(SpeechConstant.ASR_AUDIO_PATH,
// activity.getFilesDir().getPath()+"/msc/iat.wav");
// }
//
// public String voice2Text() {
// // 移动数据分析，收集开始听写事件
